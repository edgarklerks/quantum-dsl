...
> createUniverse xs = Universe $ unzip $ xs
...
> instance Monoid ( Universe a) where
>        mempty = Universe (mempty, mempty)
>        mappend (Universe (t,k)) (Universe (s, l)) = Universe (t <> s, k <> l)
...
>                    where remap g (Universe (c, pm) ) =  let (as, ds) = unzip c
>                                                             (Universe (c', pm')) = g  $ Universe (as, pm)
>                                                          in Universe (zip c' ds, normalize $ zipWith (*) pm' pm)
...
> splitEither (Universe (c, pm)) = let xs =  zip (zip c $  [0..]) pm
>                                      (ls, rs) = foldr step ([], []) xs
>                                      (uls, lpm) = unzip ls
>                                      (urs, rpm) = unzip rs
>                                  in (Universe (uls, lpm), Universe (urs, rpm))
...
> zipEither (Universe (a, pm)) (Universe (b, pm'))  = let apm = zip a pm
>                                                         bpm = zip b pm'
...
>
>                                                     in Universe (unzip xs)
> sortVectorBy f =  L.sortBy f


And split and merge universes of products:

> zipUniverse :: Universe a -> Universe b -> Universe (a,b)
> zipUniverse (Universe (a, apm)) (Universe (b, bpm)) = let ab = zip a b
>                                                           lab = length ab
>                                                       in  Universe (ab, zipWith (*) (take lab apm) (take lab bpm))

> unzipUniverse :: Universe (a,b) -> (Universe a, Universe b)
> unzipUniverse (Universe (ab, pm)) = let ~(a, b) = unzip ab
...
> pickVector :: [Double] -> RVar Int
> pickVector xs = head <$> weightedSample 1 ((fmap (\x -> abs x ^ 2) $ normalize xs) `zip` [0..])
>
...
> observeAll :: Show b => Quantum a b -> Universe a -> [(b, Double)]
> observeAll (Quantum f) a = let (Universe (b,normalize -> pm)) = f a
>                             in zip (traceShow b b) (fmap (\x -> abs x ^ 2) pm)
>
...
> normalize xs = let norm = sqrt $ L.sum $ fmap (\x -> abs x ^ 2) xs
>                in (/norm) <$> xs

Please be aware, that possibilities gets cut of if there is no universe

> entangleUniverse :: Universe b -> Quantum a (a,b)
> entangleUniverse (Universe (c,pm)) = Quantum $ \(Universe (d, dpm)) -> let xs = d `zip` c
>                                                                            l = length xs
>                                                                        in Universe (xs, zipWith (*) (take l pm) (take l dpm))
...
> universeRepeated  :: Quantum Cat (Cat, Led)
> universeRepeated = proc x -> do rec
>                                  (cat) <- returnA -< if led == Off then x else next
>                                  (next,led) <- universeRepeated -< cat
>                                 returnA -< (cat, led)


> data Experimenter = Healthy | Health Int | Death
>              deriving Show

> inputUniverse :: Universe a -> Quantum a a
> inputUniverse bs = Quantum $ \cs -> bs <> cs

> universeQuantumImmortality :: Quantum Experimenter Experimenter
> universeQuantumImmortality = proc x -> do
>                                case x of
>                                  Healthy -> do
>                                     inputUniverse (createUniverse [ (Health 90, 0.3), (Death, 0.3)]) -< x
>                                  Death -> inputUniverse (createUniverse [(Death, 1)]) -< x
>                                  otherwise -> arr (\(Health x) -> (Health (x - 10)) ) <+> inputUniverse (createUniverse [(Death, 0.3)]) <+> id -< x
